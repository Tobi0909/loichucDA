...
Một web nhỏ: mở lên là tự biết đang là buổi sáng / chiều / tối / đêm (giờ Việt Nam) và hiện một lời chúc phù hợp kèm ngày tháng, có thể lồng thêm thời tiết Hà Nội thực tế, tâm trạng tự chọn (mood picker), thỉnh thoảng có bất ngờ nhỏ kèm pháo hoa — cộng với một dòng động viên ôn thi, đồng hồ đếm ngược tới ngày thi, hiệu ứng trái tim rơi/sao lấp lánh/trăng trôi và nút bật nhạc.
...
- `DATE_PREFIXES` — phần mở đầu có ngày tháng
- `MONDAY_NOTES`, `FRIDAY_NOTES`, `SATURDAY_NOTES`, `SUNDAY_NOTES` — ý phụ theo thứ trong tuần (T2 động lực · T6 bung xoả · T7 nghỉ ngơi · CN chữa lành)
- `RAIN_NOTES`, `STORM_NOTES`, `HOT_WEATHER_NOTES`, `COLD_WEATHER_NOTES` — ghi chú thời tiết thực tế (Hà Nội), chỉ hiện khi đáng chú ý
...
Mở **[`data/moods.ts`](data/moods.ts)** để sửa nội dung theo 4 tâm trạng (`happy`/`tired`/`stressed`/`sad`, xem `lib/mood.ts`). Khi {em} bấm chọn một tâm trạng, nó **bổ sung thêm** cho lời chúc theo buổi (không thay thế) — ưu tiên hơn ghi chú theo thứ/thời tiết, và cả AI lẫn kho tĩnh đều tôn trọng lựa chọn này. Bấm lại vào tâm trạng đang chọn để bỏ chọn.

### Thời tiết thực tế

Lấy qua [Open-Meteo](https://open-meteo.com) (miễn phí, không cần API key) cho toạ độ Hà Nội — xem `lib/weather.ts`. Chỉ thêm ghi chú khi thời tiết đáng chú ý (mưa/giông/nóng gắt/lạnh), còn lại im lặng bỏ qua. Lỗi/timeout mạng thì bỏ qua hoàn toàn, không ảnh hưởng lời chúc chính.

### Bất ngờ nhỏ (easter egg)

Mở **[`data/easterEgg.ts`](data/easterEgg.ts)** để sửa. Tỉ lệ ~5% mỗi lần tải lời chúc (không áp dụng khi đã chọn tâm trạng, hoặc vào đêm trước/đúng ngày thi) sẽ thay hẳn lời chúc bằng một câu đặc biệt, kèm hiệu ứng pháo hoa nhỏ (`components/Fireworks.tsx`). Đổi tỉ lệ ở hằng số `EASTER_EGG_CHANCE` trong `app/page.tsx`.
...
│   ├── Fireworks.tsx           # Pháo hoa nhỏ cho easter egg (thuần CSS)
│   ├── MusicToggle.tsx         # Nút bật/tắt nhạc
│   └── StarField.tsx           # Sao lấp lánh + mặt trăng trôi (buổi tối/đêm)
├── data/
│   ├── easterEgg.ts            # Kho câu "bất ngờ nhỏ" (~5% tỉ lệ)
│   ├── greetings.ts            # Kho lời chúc tĩnh theo buổi/thứ/thời tiết (fallback)
│   └── moods.ts                # Kho nội dung theo tâm trạng (mood picker)
├── lib/
│   ├── config.ts               # ⚙️ Tên, xưng hô, ngày thi, chữ ký
│   ├── deck.ts                 # Shuffle kiểu "rút bài", chống lặp
│   ├── fallback.ts             # Ghép lời chúc từ kho tĩnh
│   ├── mood.ts                 # Kiểu Mood + nhãn/emoji cho mood picker
│   ├── text.ts                 # Thay token {em}/{anh}/{date}
│   ├── time.ts                 # Giờ Asia/Ho_Chi_Minh, phân buổi, thứ/ngày
│   └── weather.ts              # Thời tiết Hà Nội qua Open-Meteo (không cần key)
├── public/
